...
def sync_gradients_gather_scatter(model, world_size, rank):
    """Synchronize gradients across workers using gather and scatter.
    
    Worker 0 gathers gradients from all workers, averages them,
    then scatters the averaged gradients back to all workers.
    """
    for param in model.parameters():
        if param.grad is not None:
            ...
            if rank == 0:
                # Rank 0: gather gradients from all workers
                ...
                # Scatter averaged gradient to all workers
                scatter_list = [avg_grad.clone() for _ in range(world_size)]
                dist.scatter(grad, scatter_list=scatter_list, src=0)
            else:
                # Non-root: send gradient to rank 0
                dist.gather(grad, dst=0)

                # Receive averaged gradient from rank 0
                dist.scatter(grad, src=0)

            # Update the gradient with the synchronized value
            param.grad.data = grad


def train(args, train_dataset, model, tokenizer):
    """ Train the model with distributed data parallel using gather/scatter """
    ...
    # Tracking for results saving
    step_losses = []       # (step, loss_value) for loss curve
    step_times = []        # time per iteration (skip first)

    set_seed(args)  # Added here for reproducibility (even between python 2 and 3)

    # ---- Task 4: Profiling setup ----
    # Profile 3 training steps, skipping the first step
    ...
    trace_file = os.path.join(trace_dir, f"task2a_rank{args.local_rank}.json")
        ...
        for epoch in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if args.fp16:
                    ...
                    # Synchronize gradients across workers using gather/scatter
                    sync_gradients_gather_scatter(model, args.world_size, args.local_rank)
                ...
                # Record iteration time (skip first iteration for timing)
                ...
                if step < 5:
                    print(f"[Rank {args.local_rank}] Step {step}, Loss: {loss.item():.6f}")
            ...
            # Evaluate after every epoch (only on rank 0 to avoid duplicate output)
            if args.local_rank == 0:
                evaluate(args, model, tokenizer, prefix="")

    # Export chrome trace for Task 4
    ...
    # Save loss data (for loss curves)
    loss_file = os.path.join(results_dir, f"losses_rank{args.local_rank}.json")
    ...
    logger.info(f"Loss data saved to {loss_file}")

    # Save timing data
    avg_time = sum(step_times) / len(step_times) if step_times else 0.0
    timing_file = os.path.join(results_dir, f"timing_rank{args.local_rank}.json")
    with open(timing_file, "w") as f:
        json.dump({
            "rank": args.local_rank,
            "world_size": args.world_size,
            "method": "gather_scatter",
            "num_iterations": len(step_times) + 1,  # +1 for skipped first
            "num_timed_iterations": len(step_times),
            "average_time_per_iteration": avg_time,
            "total_timed_time": sum(step_times),
            "all_iteration_times": step_times
        }, f, indent=2)
    logger.info(f"Timing data saved to {timing_file}")

    print(f"\n[Rank {args.local_rank}] Average time per iteration (excluding first): {avg_time:.4f}s")
    print(f"[Rank {args.local_rank}] Total iterations: {len(step_times) + 1}, Timed iterations: {len(step_times)}")
...
def main():
    ...
    parser.add_argument("--local_rank", type=int, default=-1,
                        help="For distributed training: local_rank (node ID).")
    ...
    print(f"[Rank {args.local_rank}] Distributed initialized: world_size={args.world_size}, backend=gloo")
