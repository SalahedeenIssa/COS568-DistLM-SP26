...
def sync_gradients_all_reduce(model, world_size):
    """Synchronize gradients across workers using all_reduce.
    
    Performs all_reduce with SUM operation on each parameter's gradient,
    then divides by world_size to get the average.
    """
...
def train(args, train_dataset, model, tokenizer):
    """ Train the model with distributed data parallel using all_reduce """
    ...
    trace_file = os.path.join(trace_dir, f"task2b_rank{args.local_rank}.json")
